function Af_EditFast(lines, edits, newname, defname)

%% Open Files
fidDefault  = fopen([defname,'.fst'],'r');
if fidDefault==-1
    error(['Error: ', defname, '.fst not found.  Note: you do not need to end string with .fst']);
end
fidNew      = fopen([newname,'.fst'],'w+');
if fidNew==-1
...
if isempty(edits)
    ...
    copyfile([defname,'.fst'],[newname,'.fst']);
else
...
fclose('all');
disp(['Status: ', newname,'.fst created with ',num2str(length(edits)),' changed input(s).']);
