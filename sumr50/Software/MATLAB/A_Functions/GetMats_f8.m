...
for ii = 1:NAzimStep
    data(ii) = ReadFASTLinear(FileNames{ii}); %we'll read this twice so we can allocate space first; putting it at NAzimStep saves some reallocation later
end
...
for i = 1:NActvDOF	% loop through all active (enabled) DOFs
    if data(1).x_rotFrame(i)  % this is a state in the rotating frame
        ...
        col = strfind(data(1).x_desc{i},'blade');                    % find the starting index of the string 'blade'
        if ~isempty(col)             % true if the DescStates{I} contains the string 'blade'
            k = str2double(data(1).x_desc{i}(col+6));                % save the blade number for the initial blade
            Tmp = zeros(1,2);                                       % first initialize to zero
            Tmp(k) = i;                                           % save the index for the initial blade
            for j = (i+1):NActvDOF                                  % loop through all remaining active (enabled) DOFs
                if strncmp(data(1).x_desc{j},data(1).x_desc{i},col)      % true if we have the same state from a different blade
                    k = str2double(data(1).x_desc{j}(col+6));        % save the blade numbers for the remaining blades
...
%   rotating frame:
if (NInputs > 0)
    [RotTripletIndicesCntrlInpt, NRotTripletCntrlInpt] = findBladeTriplets(data(1).u_rotFrame,data(1).u_desc);
end
...
%   rotating frame:
if (NumOuts > 0 )
%       [RotTripletIndicesOutput, NRotTripletOutput] = findBladeTriplets(data(1).y_rotFrame,data(1).y_desc,{'ED Root...\d, (kN)','ED RootM..\d'});
