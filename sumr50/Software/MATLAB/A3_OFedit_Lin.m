...
if ~exist('FA','var')
%   FA.Linearize  = 'True';
%   FA.LinTimes   = Linear.TimesString;
%   FA.NLinTimes  = length(Linear.LinTimes);  % Edit Script is reading the field twice 
end
FA.CompAero   = 2;
FA.CompElast  = 1;
FA.CompServo  = 1;
...
ED.FlapDOF1     = DOF.FlapDOF1;
ED.FlapDOF2     = DOF.FlapDOF2;
ED.EdgeDOF      = DOF.EdgeDOF;
ED.TeetDOF      = DOF.TeetDOF;
ED.DrTrDOF      = DOF.DrTrDOF;
ED.GenDOF       = DOF.GenDOF;
ED.YawDOF       = DOF.YawDOF;
ED.TwFADOF1     = DOF.TwFADOF1;
ED.TwFADOF2     = DOF.TwFADOF2;
ED.TwSSDOF1     = DOF.TwSSDOF1;
ED.TwSSDOF2     = DOF.TwSSDOF2;
...
SD.PCMode    = num2str(0);
SD.YCMode    = num2str(0);
SD.VSContrl = num2str(1);
SD.GenTiStr  = 'True';
SD.VS_RtGnSp = Parameters.Turbine.wg_rated;
SD.VS_Rgn2K  = round(Parameters.Turbine.T_rated/Parameters.Turbine.wg_rated^2);
SD.VS_RtTq   = Parameters.Turbine.T_rated;
SD.HSSBrMode = num2str(0);
...
IW.WindType   = num2str(1);
IW.HWindSpeed = num2str(A_Wspd(iOpPt));
IW.PLexp      = num2str(0);
...
clear FASTList FASTEditLine FASTEditValue i var_check AeroEditNames BeamEditNames ElastEditNames ...
      FastEditNames InflowEditNames Name_AD* Name_BD* Name_ED* Name_IW* Name_SD* ServoEditNames