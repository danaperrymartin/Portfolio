...
namespace TCPServerFTP
{
    public class FTP
    {
        ...
        public delegate void FTPClientProgressBarEventHandler(int progress);
        public event FTPClientProgressBarEventHandler FTPClientProgressBarEvent;

        public delegate void FTPClientFileNameEventHandler(string filename);
        public event FTPClientFileNameEventHandler FTPClientFileNameEvent;

        public DateTime GetLastLastHB
        {
            set { tcp_server.dtLastHB = value; }
            get { return tcp_server.dtLastHB; }
        }
        //---------------------------------------------------------------------------------------------------------------------------------------------
        //  PRIVATE
        //---------------------------------------------------------------------------------------------------------------------------------------------
        public TCPServer tcp_server{ set; get; }
        ...
        public FTP()
        {
            ...
            if (tcp_server != null)
            {
                tcp_server.dtLastHB = DateTime.Now;
            }
        }
        ...
        public Tuple<byte[], bool> getData(TcpClient client)
        {
            ...
            while (bytesToRead > 0)
            {
                ...
                //{
                RaiseFTPProgressBarEvent(Convert.ToInt32(packet_percent));
                //}
        ...
        private void RaiseFTPProgressBarEvent(int progress)
        {
            if (FTPClientProgressBarEvent != null)
            {
                FTPClientProgressBarEvent(progress);
            }
        }

        private void RaiseFTPFileNameEvent(string filename)
        {
            if (FTPClientFileNameEvent != null)
            {
                FTPClientFileNameEvent(filename);
            }
        }
    }
}
