...
cv::Mat ImageSegmentation::Predict(Mat im, vector<Rect>& rects, cv::Size IMAGE_SIZE, vector<string> label)
{
    ...
    dnn::Net net = dnn::readNetFromONNX("C:/CodeProjects/MachineVision/SpringFinder/SpringFinder/models/Trucks/pix2pix_10epoch.onnx");
...
cv::Mat ImageSegmentation::FrequencySegmentation(Mat& im, vector<Rect>& rects)
{
    Mat input = im;
    ...
    // construct H
    Mat H;
    H = ff.construct_H(magI, "Gaussian", 100);
    ...
    texture.displayImage("Input Image", input);
    texture.displayImage("spectrum magnitude", input);
    texture.displayImage("filtered image", input);

    return imgOut;
}

void ImageSegmentation::TemplateSegmentation(Mat& img, Mat& templ)
{
    Mat img_display;
    Mat result;
    Mat mask;
    ...
    TextureHelperFunctions texture;
    texture.displayImage("ImageWindow", img_display);
    //texture.displayImage("ResultWindow", result);

    texture.saveImage("TemplateMatching", img_display);
    return;
}