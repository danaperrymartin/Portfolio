...
typedef map<int, int> HDM;
typedef vector<Point> Contour;
typedef vector<Contour> Contours;


void TextureHelperFunctions::displayImage(string const& windowName, Mat& image, bool withExit)
{
...
void TextureHelperFunctions::saveImage(string const& windowName, Mat image)
{
    cv::imwrite(windowName + ".jpg", image);
}

template<typename T>
void TextureHelperFunctions::getIteratorAtPoint(Mat const& image, Point const& p, MatIterator_<T>& it)
{
...
template<typename T>
MatIterator_<T> TextureHelperFunctions::getIteratorAtPoint(Mat& image, Point const& p)
{
    Size iSize = image.size();
...
template<typename T>
MatConstIterator_<T> TextureHelperFunctions::getIteratorAtPoint(Mat const& image, Point const& p)
{
    Size iSize = image.size();
    return (image.begin<T>() + p.x * iSize.width + p.y);
}
...
void TextureHelperFunctions::removeColumn(Mat const& image, Mat& histogram, Point const& p, map<int,int>& m)
{
...
void TextureHelperFunctions::addColumn(Mat const& image, Mat& histogram, Point const& p, map<int, int>& m)
{
    MatConstIterator_<uchar> it;
    MatConstIterator_<uchar> begin = TextureHelperFunctions::getIteratorAtPoint<uchar>(image, p);
    MatConstIterator_<uchar> end = begin + HISTOGRAM_DIMENSION * image.cols;
    HDM::iterator itHDM;
...
void TextureHelperFunctions::removeRow(Mat const& image, Mat& histogram, Point const& p, map<int, int>& m)
{
    MatConstIterator_<uchar> it;
    MatConstIterator_<uchar> begin = TextureHelperFunctions::getIteratorAtPoint<uchar>(image, p);
...
void TextureHelperFunctions::addRow(Mat const& image, Mat& histogram, Point const& p, map<int, int>& m)
{
    MatConstIterator_<uchar> it;
    MatConstIterator_<uchar> begin = TextureHelperFunctions::getIteratorAtPoint<uchar>(image, p);
    MatConstIterator_<uchar> end = begin + HISTOGRAM_DIMENSION;
...
double TextureHelperFunctions::calculateEntropy(Mat const& histogram)
{
    double entropyVal = 0;
...
double TextureHelperFunctions::calculateEntropyAndUpdateHistogram(map<int, int>& m, double previousPixelEntropyValue, Mat& histogram)
{
...
void TextureHelperFunctions::calculateInitialHistogram(Mat const& image, Mat& histogram)
{
    Size iSize = image.size();
...
void TextureHelperFunctions::entropyFilt(Mat const& image, Mat& entropy)
{
...
void TextureHelperFunctions::calculateSymetricPoint(Point const& inPoint, Point& outPoint, Size const& psize)
{
...
void TextureHelperFunctions::symmetricPadding(Mat& image, Mat& paddedImage)
{
    //Padd with zeros
...
void TextureHelperFunctions::bwareaopen(Mat const& input, Mat& output)
{
    // Find all contours
    Contours contours;
    ...
    for (int i = 0; i < contours.size(); i++)
    {
        ...
        if (area > TextureHelperFunctions::AREA_THRESHOLD)
            drawContours(output, contours, i, 255, -1);
    }
}

void TextureHelperFunctions::imclose(Mat& image)
{
    morphologyEx(image, image, MORPH_CLOSE, cv::getStructuringElement(MORPH_RECT, Size(9, 9)));
}

void TextureHelperFunctions::imfill(Mat& image)
{
...
void TextureHelperFunctions::maskImage(Mat const& image, Mat const& mask, Mat& output)
{
    output = image & mask;
}

void TextureHelperFunctions::conversion(Mat const& input, Mat& output)
{
