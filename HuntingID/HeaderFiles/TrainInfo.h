...
class TrainInfo
{
	...
	public:
        ...
        struct Wheel_Info
        {
            double* WheelForce_PeakTime = new double[16]{ 0.0 };
            double* WheelForce_Vert     = new double[16]{ 0.0 };
            double* WheelForce_Lat      = new double[16]{ 0.0 };
            double* WheelForce_Static   = new double[16]{ 0.0 };
            double* WheelForce_NormLat  = new double[16]{ 0.0 };
        };
        ...
        std::vector<TrainInfo::Wheel_Info> Wheel_Forces_Near_Array;
        std::vector<TrainInfo::Wheel_Info> Wheel_Forces_Far_Array;
        ...
        ~TrainInfo()
        {
            ...
            Trn_Forces.clear();
            Cars.clear();
            Wheel_Forces_Near.clear();
            Wheel_Forces_Far.clear();
            currenttrain_list.clear();

            Wheel_Forces_Near_Array.clear();
            Wheel_Forces_Far_Array.clear();
        };

        void ClearTrainInfo()
        {
            Trn_Car.clear();
            Trn_Car.shrink_to_fit();

            Trn_Whl.clear();
            Trn_Whl.shrink_to_fit();

            Trn_Forces.clear();
            Trn_Forces.shrink_to_fit();

            Cars.clear();
            Cars.shrink_to_fit();

            Wheel_Forces_Near.clear();
            Wheel_Forces_Near.shrink_to_fit();

            Wheel_Forces_Far.clear();
            Wheel_Forces_Far.shrink_to_fit();

            currenttrain_list.clear();
            currenttrain_list.shrink_to_fit();

            Wheel_Forces_Near_Array.clear();
            Wheel_Forces_Near_Array.shrink_to_fit();

            Wheel_Forces_Far_Array.clear();
            Wheel_Forces_Far_Array.shrink_to_fit();
        }
        ...
        void Reshape4Car(std::string ifile)
        {
            ...
            for (int icar = 0; icar < Cars.size(); icar++)
            {
                    ...
                    outfile_near.open(directory_name + Cars[icar].Car_Owner + Cars[icar].Car_Number + "_" + to_string(Cars[icar].Car_Axles) + "_Near" + ".hdat");

                    ofstream outfile_far;
                    outfile_far.open(directory_name + Cars[icar].Car_Owner + Cars[icar].Car_Number + "_" + to_string(Cars[icar].Car_Axles) + "_Far" + ".hdat");
        ...
        void Reshape4Wheel()
        {
            ...
            for (int icar = 0; icar < Cars.size(); icar++)
            {
                for (int iaxle = 0; iaxle < (int)(Cars[icar].Car_Axles); iaxle++)
                {
                    TrainInfo::Wheel_Info current_whl;
                    Wheel_Forces_Near_Array.push_back(current_whl);
                    Wheel_Forces_Far_Array.push_back(current_whl);
                    for (int icrib = 0; icrib < Trn_Forces.size(); icrib++)
                    {
                        if (iaxle_total<Trn_Forces[icrib].WheelForce_PeakTime.size())
                        {
                            ...
                            if (Trn_Forces[icrib].Crib_Location.compare("Near") == 0)
                            {
                                ...
                                Wheel_Forces_Near_Array[iaxle_total].WheelForce_PeakTime[icount] = Trn_Forces[icrib].WheelForce_PeakTime[iaxle_total];
                                Wheel_Forces_Near_Array[iaxle_total].WheelForce_Lat[icount]      = Trn_Forces[icrib].WheelForce_Lat[iaxle_total];
                                Wheel_Forces_Near_Array[iaxle_total].WheelForce_Static[icount]   = Trn_Forces[icrib].WheelForce_Static[iaxle_total];
                                Wheel_Forces_Near_Array[iaxle_total].WheelForce_NormLat[icount]  = Trn_Forces[icrib].WheelForce_Lat[iaxle_total] / Trn_Forces[icrib].WheelForce_Static[iaxle_total];
                                icount++;
                            }
                            else if (Trn_Forces[icrib].Crib_Location.compare("Far") == 0)
                            {
                                ...
                                Wheel_Forces_Far_Array[iaxle_total].WheelForce_PeakTime[icount] = Trn_Forces[icrib].WheelForce_PeakTime[iaxle_total];
                                Wheel_Forces_Far_Array[iaxle_total].WheelForce_Lat[icount]      = Trn_Forces[icrib].WheelForce_Lat[iaxle_total];
                                Wheel_Forces_Far_Array[iaxle_total].WheelForce_Static[icount]   = Trn_Forces[icrib].WheelForce_Static[iaxle_total];
                                Wheel_Forces_Far_Array[iaxle_total].WheelForce_NormLat[icount]  = Trn_Forces[icrib].WheelForce_Lat[iaxle_total] / Trn_Forces[icrib].WheelForce_Static[iaxle_total];
                                icount++;
                            }
                        }
                    }
        ...
        void LengthNotEqual()
        {
            ...
            size_t tempCount = 1, mode = 1, lastNumber = vector_length[0];
...
public:
    ...
    TrainInfoHandle() : traininfo_h(new TrainInfo()) {  }

    void terminateTrainInfoHandle()
    {
        traininfo_h = NULL;
    }
    ...
    TrainInfo* operator->() { return traininfo_h; }
};